...
This file is bundled with every FrontPress Studio install under `.Codex/skills/frontpress-cms/`. Open the install's root directory in **Codex** (the Codex CLI/desktop dev tool) and this skill auto-loads whenever the conversation touches framework internals. Users who run Codex in another tool can copy this folder into their `~/.Codex/skills/` to load it globally.

No "connect site folder" step is required — the working directory of your Codex session IS the site folder. If Codex is in `app/public/` of your install, every path reference below is already resolved.
...
8. **File size budget** — project rule: no source file in `src/` or `cms/lib/` over 300 lines. Split before you hit it. See repo `AGENTS.md` for the canonical splits.
...
## File size rule (from project AGENTS.md)
