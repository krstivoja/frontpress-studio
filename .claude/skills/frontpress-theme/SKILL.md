...
| `partial(name, vars={})` | includes a partial | `{{ partial('card', { post: p })|raw }}` |
...
tags: [news, release]
categories: [updates]
image: /uploads/blog/foo/hero.png   # featured image; string OR [path, alt]
...
URLs resolve via the `/uploads/*` route — `index.php` looks under `site/content/` first, then `site/uploads/`. Reference as `/uploads/blog/hello-world/hero.png` in Markdown / templates.
